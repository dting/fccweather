(function() {
  ...
  function init() {
    $.get('http://freegeoip.net/json/', function(response) {
      ...
      getWeather(coordinates, getUnits(response.country_code));
      $('#region').html(response.region_name);
  ...
  $('#tempContainer').click(function toggleFC() {
    var tempDiv = document.getElementById('temp');
    var tu = tempDiv.innerHTML.split('°');
    if (tu[1] === 'F') {
      tempDiv.innerHTML = ((+tu[0] - 32) / 1.8).toFixed(2) + '&deg;' + 'C';
    } else {
      tempDiv.innerHTML = ((+tu[0] * 1.8) + 32).toFixed(2) + '&deg;' + 'F';
    }
  });
  ...
  function getWeather(coords, units) {
    var weatherAPI = 'http://api.openweathermap.org/data/2.5/weather?lat=' +
                     coords.lat + '&lon=' + coords.lon + '&units=' + units;
    $.get(weatherAPI, function(weather) {
      $('#weatherIcon').html('<img src="http://openweathermap.org/img/w/' +
                             weather.weather[0].icon + '.png">');
      $('#temp').html(weather.main.temp + '&deg;' +
                      (units === 'metric' ? 'C' : 'F'));
      var description = DESCRIPTIONS[weather.weather[0].icon];
      $('#description').html(description.text);
      $('body').css('backgroundImage', 'url("' + description.img + '")');
      $('#attribution').html(attribution(description));
      $('#windDirection').html(degToCompass(weather.wind.deg));
      $('#windSpeed').html(weather.wind.speed + ' knots');
      $('.loadable').fadeIn();
    }, 'jsonp');
  }

  function attribution(description) {
    return '<a href="' + description.attr + '">' + description.attr + '</a>';
  }
